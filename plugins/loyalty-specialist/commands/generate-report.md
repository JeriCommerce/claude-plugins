...
4. Execute **Step 1** from the SQL queries to identify the program. If the input looks like a UUID, query by `id`. If it contains only lowercase letters and hyphens, query by `slug`. Otherwise, search by name with `ILIKE`. If multiple results are found, ask the user to confirm.

5. Execute **Steps 2–16** sequentially using the confirmed program UUID. Use Metabase with `database_id: 3` for all queries. Collect ALL results before proceeding.
